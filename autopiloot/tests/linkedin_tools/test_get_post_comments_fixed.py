"""
Fixed comprehensive tests for GetPostComments tool targeting 100% coverage.
Tests all methods, error paths, and edge cases with proper mocking.
"""
...
class TestGetPostCommentsFixed(unittest.TestCase):
    """Fixed comprehensive test coverage for GetPostComments tool."""
    ...
    def test_successful_comments_fetch_lines_74_158(self):
        """Test successful comments fetch covering main run method (lines 74-158)."""
        with patch.dict('sys.modules', {
            'agency_swarm': MagicMock(),
            'agency_swarm.tools': MagicMock(),
            'pydantic': MagicMock()
            ...
            with patch('linkedin_agent.tools.get_post_comments.get_required_env_var') as mock_env, \
                ...
                # Mock successful API response through _make_request_with_retry
                mock_response_data = {
                    "data": [
                        {
                            "id": "comment_1",
                ...
                with patch.object(tool, '_make_request_with_retry', return_value=mock_response_data):
                    ...
                    # Verify main response structure (lines 146-156)
                    self.assertIn("comments_by_post", result_data)
                    self.assertIn("metadata", result_data)

                    # Verify post comments (lines 125-131)
                    ...
                    self.assertEqual(post_comments["page"], 1)

                    # Verify metadata (lines 148-155)
                    ...
                    self.assertTrue(metadata["include_replies"])

    def test_page_size_validation_lines_83_84(self):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('linkedin_agent.tools.get_post_comments.get_required_env_var') as mock_env, \
                ...
                with patch.object(tool, '_make_request_with_retry', return_value={"data": [], "pagination": {}}):
                    result = tool.run()
                    result_data = json.loads(result)

                    # Verify page_size was clamped to 100 (line 84)
    ...
    def test_empty_post_ids_error_lines_86_90(self):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('linkedin_agent.tools.get_post_comments.get_required_env_var') as mock_env, \
                ...
                # Verify empty post IDs error (lines 87-90)
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('linkedin_agent.tools.get_post_comments.get_required_env_var') as mock_env, \
                ...
                tool = GetPostComments(
                    ...
                    include_replies=False  # Test include_replies=False path (line 112 condition)
                )
                ...
                with patch.object(tool, '_make_request_with_retry', return_value=mock_response_data):
                    ...
                    # Verify both posts processed (lines 104-143)
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('linkedin_agent.tools.get_post_comments.get_required_env_var') as mock_env, \
                ...
                with patch.object(tool, '_make_request_with_retry', return_value=None):
                    ...
                    # Verify failed post handling (lines 134-139)
                    post_comments = result_data["comments_by_post"]["urn:li:activity:123"]
    ...
    def test_exception_handling_lines_160_166(self):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('linkedin_agent.tools.get_post_comments.load_environment') as mock_load:
                ...
                # Verify exception handling (lines 161-166)
    ...
    def test_process_comments_comprehensive_lines_178_201(self):
        ...
        with patch.dict('sys.modules', {
            ...
            comments = [
                ...
                {
                    # Test comment with missing fields (lines 182-193 default values)
                    "id": "comment_2"
                }
            ]
            ...
            # Verify first comment processing (lines 181-193)
            ...
            # Verify replies are processed recursively (lines 196-197)
    ...
    def test_process_comments_without_replies_line_196(self):
        """Test _process_comments without replies (line 196 condition)."""
        with patch.dict('sys.modules', {
            'agency_swarm': MagicMock(),
            'agency_swarm.tools': MagicMock(),
            'pydantic': MagicMock()
            ...
            # Verify replies are not included when include_replies=False (line 196)
    ...
    def test_make_request_with_retry_success_lines_216_224(self):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('linkedin_agent.tools.get_post_comments.requests.get') as mock_get:
                ...
                # Verify success path (lines 223-224)
    ...
    def test_make_request_with_retry_rate_limiting_lines_226_231(self):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('linkedin_agent.tools.get_post_comments.requests.get') as mock_get, \
                ...
                # Verify rate limiting sleep was called (line 229)
    ...
    def test_make_request_with_retry_server_errors_lines_233_237(self):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('linkedin_agent.tools.get_post_comments.requests.get') as mock_get, \
                ...
                # Verify server error retry with backoff (lines 235-237)
    ...
    def test_make_request_with_retry_client_errors_lines_239_242(self):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('linkedin_agent.tools.get_post_comments.requests.get') as mock_get, \
                ...
                # Verify client error handling (lines 241-242)
    ...
    def test_make_request_with_retry_timeout_lines_244_247(self):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('linkedin_agent.tools.get_post_comments.requests.get') as mock_get, \
                ...
                # Verify timeout handling (lines 245-247)
    ...
    def test_make_request_with_retry_request_exception_lines_248_251(self):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('linkedin_agent.tools.get_post_comments.requests.get') as mock_get, \
                ...
                # Verify exception handling (lines 249-251)
    ...
    def test_make_request_with_retry_all_retries_failed_line_253(self):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('linkedin_agent.tools.get_post_comments.requests.get') as mock_get, \
                ...
                # Verify None is returned after all retries fail (line 253)
                self.assertIsNone(result)
                self.assertEqual(mock_get.call_count, 2)

    def test_include_replies_parameter_lines_112_113(self):
        ...
        with patch.dict('sys.modules', {
            ...
            tool = GetPostComments(post_ids=["test"], include_replies=True)

            # Mock the actual request call to verify parameters
            with patch('linkedin_agent.tools.get_post_comments.requests.get') as mock_get:
                mock_response = Mock()
                mock_response.status_code = 200
                ...
                tool._make_request_with_retry(
                    "https://test-api.com",
                    {"Authorization": "Bearer test"},
                    {"postId": "test", "includeReplies": "true"}
                )

                # Verify request was made with includeReplies parameter
                mock_get.assert_called_once()
