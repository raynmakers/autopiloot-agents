"""
Fixed comprehensive tests for GetUserPosts tool targeting high coverage.
...
class TestGetUserPostsFixed(unittest.TestCase):
    """Fixed comprehensive test coverage for GetUserPosts tool."""
    ...
    def test_successful_posts_fetch_single_page_lines_77_163(self):
        """Test successful posts fetch for single page covering main run method (lines 77-163)."""
        with patch.dict('sys.modules', {
            'agency_swarm': MagicMock(),
            'agency_swarm.tools': MagicMock(),
            'pydantic': MagicMock()
    ...
    def test_since_iso_parameter_lines_106_107_160_161(self):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('linkedin_agent.tools.get_user_posts.get_required_env_var') as mock_env, \
                ...
                with patch.object(tool, '_make_request_with_retry', return_value=mock_response_data) as mock_request:
                    result = tool.run()
                    result_data = json.loads(result)

                    # Verify since parameter was used in request (checked by mock_request call)
