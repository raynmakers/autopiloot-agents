...
All chunking, hashing, and OpenSearch indexing logic has been moved to the core library.
...
class IndexFullTranscriptToOpenSearch(BaseTool):
    """
    DEPRECATED: Index full transcript chunks to OpenSearch via shared core library.
    ...
    - Parallel ingestion to OpenSearch (and other sinks if enabled)
    ...
    def run(self) -> str:
        """
        Delegate to shared core library for transcript indexing.

        Returns:
            JSON string with index_name, chunk_count, indexing status
        """
        print("⚠️ DEPRECATION WARNING: IndexFullTranscriptToOpenSearch is deprecated.")
        ...
        try:
            ...
            if result.get("status") == "success":
                # Extract OpenSearch-specific results if available
                opensearch_result = result.get("sinks", {}).get("opensearch", {})

                return json.dumps({
                    "index_name": opensearch_result.get("index_name", "autopiloot_transcripts"),
                    "video_id": self.video_id,
                    "chunk_count": result.get("chunk_count", 0),
                    "indexed_count": result.get("chunk_count", 0),
                    "error_count": 0,
                    "content_hashes": result.get("content_hashes", []),
                    "total_tokens": result.get("total_tokens", 0),
                    "status": "indexed",
                    "message": result.get("message", "Indexed via core library")
                }, indent=2)

            elif result.get("status") == "skipped":
                return json.dumps({
                    "status": "skipped",
                    "message": result.get("message", "OpenSearch not configured"),
                    "video_id": self.video_id
                }, indent=2)

            else:
                return json.dumps({
                    "error": result.get("error", "indexing_failed"),
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("TEST: Index full transcript to OpenSearch (DEPRECATED SHIM)")
    print("="*80)
